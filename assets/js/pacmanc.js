...
function drawPacman() { 
	...
	var gradient = ctx.createRadialGradient(PACMAN_POSITION_X, PACMAN_POSITION_Y, 0, PACMAN_POSITION_X, PACMAN_POSITION_Y, PACMAN_SIZE);
    gradient.addColorStop(0, '#FF4500'); // vermelho
			...
			if ( bubbleParams[4] === "0" ) { 
				...
				if ( type === "s" ) { 
					...
					score( SCORE_SUPER_BUBBLE );
													           			playAudioAtScore(0, 8); // Amarelinho, vaza do labirinto!
					// playEatPillSound();
					affraidGhosts();
				} else { 
					score( SCORE_BUBBLE );
