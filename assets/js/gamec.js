...
function initGame(newgame) { 
	...
	if (newgame) { 
		...
		var ctx = null;
		var canvas = document.getElementById('tittleGame');
...
function win() {
	postData("/setcoins", { username: USERNAME, score: SCORE}).then((data) => {
		console.log(data); 
		}); 
	stopAllSound();
	console.log("Ganhei");
...
function gameover() { 
	//console.log(USERNAME + " morreu " + SCORE)
	

	GAMEOVER = true;
	message("game over");
	pauseAudio(8);
	playAudioAtScore(0, 5); // Você está morto Pac!
	stopTimes();

	erasePacman();
	eraseGhosts();
//------------------------------------------------------------------------------------------------------------Fala do vilão quando pegar o Pac
...
function message(m) { 
	...
	$("#message").html(m);
	if (m === "game over") $("#message").addClass("red") && $("#message").css("box-shadow", "inset 0 0 1000vw rgba(254, 0, 0, 0.732)");
}
...
//================================================================================================== falas personagem
	...
	if(SCORE === 10){
		playAudioAtScore(10, 7); // Chega! vamos terminar logo com isso
	}
	if(SCORE === 500){
		playAudioAtScore(500, 4); // Ei, bolinha amarela, você me lembra os seus pais. Conheceu meus pais? Sim, eles eram tão ingenuos quanto você
	}
	if(SCORE === 1600){
		playAudioAtScore(1600, 0); // Eita Pac doidão.
	}
	if(SCORE === 2200){
		playAudioAtScore(2000, 1); // Ah, seu amarelinho doidão.
	}
	if(SCORE === 2600){
		playAudioAtScore(2400, 2); // Amarelinho, vaza do labirinto!
	}
	if(SCORE === 3100){
		playAudioAtScore(3100, 3); // Ah, alguem tem uma pastilha ai?
	}

	
//============================================================================================================= Fim das falas do personagem
			else { 
